...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_safety_policy_sets_key_version_status",
        "safety_policy_sets",
        ["key", "version", "status"],
    )
    op.create_table(
    ...
    op.create_index(
        "ix_safety_policy_evaluations_workspace_created_at",
        "safety_policy_evaluations",
        ["workspace_id", "created_at"],
    )
    op.create_index(
        "ix_safety_policy_evaluations_source_type_source_id",
        "safety_policy_evaluations",
        ["source_type", "source_id"],
    )
    op.create_index(
        "ix_safety_policy_evaluations_safety_status", "safety_policy_evaluations", ["safety_status"]
    )


def downgrade() -> None:
    op.drop_index(
        "ix_safety_policy_evaluations_safety_status", table_name="safety_policy_evaluations"
    )
    op.drop_index(
        "ix_safety_policy_evaluations_source_type_source_id", table_name="safety_policy_evaluations"
    )
    op.drop_index(
        "ix_safety_policy_evaluations_workspace_created_at", table_name="safety_policy_evaluations"
    )
