...
@router.post("/analysis-runs/{analysis_run_id}/market-session", response_model=MarketSessionContextRead)
async def create_analysis_run_market_session(
    ...
    return MarketSessionContextRead.model_validate(await service.create_for_analysis_run(analysis_run_id))


@router.get("/analysis-runs/{analysis_run_id}/market-session", response_model=MarketSessionContextRead)
async def get_analysis_run_market_session(
    analysis_run_id: UUID,
    service: Annotated[MarketSessionService, Depends(get_market_session_service)],
) -> MarketSessionContextRead:
    return MarketSessionContextRead.model_validate(await service.get_for_analysis_run(analysis_run_id))


@router.post("/signals/{signal_id}/market-session", response_model=MarketSessionContextRead)
