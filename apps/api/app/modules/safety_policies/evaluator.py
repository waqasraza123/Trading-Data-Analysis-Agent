...
class SafetyPolicyEvaluator:
    def evaluate_text(
        ...
        findings.extend(self._find_phrase_matches(text, active_policy.rules.unsafe_direct_phrases, "unsafe_direct_phrase", SafetyFindingSeverity.HIGH, "Direct trading or unsafe certainty language is not allowed."))
        findings.extend(self._find_phrase_matches(text, active_policy.rules.causation_phrases, "causation_claim", SafetyFindingSeverity.MEDIUM, "Unsupported causal certainty should be reviewed."))
        findings.extend(self._find_phrase_matches(text, active_policy.rules.invented_evidence_phrases, "invented_evidence_claim", SafetyFindingSeverity.MEDIUM, "Evidence certainty claims should be grounded before public use."))
        findings.extend(self._find_phrase_matches(text, active_policy.rules.prohibited_output_claims, "prohibited_output_claim", SafetyFindingSeverity.HIGH, "The output contains a prohibited claim."))
    ...
    def evaluate_action(
        ...
        blocked_actions = {self._normalize_action(value) for value in active_policy.rules.blocked_trading_actions}
        findings: list[SafetyFinding] = []
    ...
    def evaluate_payload(
        ...
        safety_status = SafetyStatus.REDACTED if redacted_output is not None else SafetyStatus.PASSED
    ...
    def evaluate_report(self, report: str, policy: SafetyPolicy | None = None) -> SafetyEvaluationResponse:
        return self.evaluate_text(report, policy, SafetyPolicyEvaluationType.REPORT)

    def evaluate_reasoning_output(self, output: str, policy: SafetyPolicy | None = None) -> SafetyEvaluationResponse:
        return self.evaluate_text(output, policy, SafetyPolicyEvaluationType.REASONING_OUTPUT)

    def evaluate_webhook_payload(self, payload: Mapping[str, Any], policy: SafetyPolicy | None = None) -> SafetyEvaluationResponse:
        return self.evaluate_payload(payload, policy, SafetyPolicyEvaluationType.WEBHOOK_PAYLOAD, public_response=True)

    def evaluate_dataset_record(self, payload: Mapping[str, Any], policy: SafetyPolicy | None = None) -> SafetyEvaluationResponse:
        return self.evaluate_payload(payload, policy, SafetyPolicyEvaluationType.DATASET_RECORD, public_response=False)

    def redact_payload(self, payload: Any, policy: SafetyPolicy | None = None) -> Any:
        active_policy = policy or default_policy()
            ...
            if public_response and is_sensitive_key(key_text, rules.provider_payload_exposure_keys):
                findings.append(
                    SafetyFinding(
                        ...
                        message="Provider payload internals must not be exposed in public responses.",
                        matchedValue=key_text,
                        location=location,
                    )
                )
            if isinstance(value, Mapping):
                findings.extend(self._find_payload_findings(value, rules, public_response, location))
            elif isinstance(value, list):
                findings.extend(self._find_payload_sequence_findings(value, rules, public_response, location))
        return findings
