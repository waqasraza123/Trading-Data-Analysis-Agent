...
class NotificationWorkerRuntime:
    def __init__(
        ...
        self.heartbeat = RuntimeWorkerHeartbeatClient(
            ...
            settings=settings,
            worker_definition_key="notification_delivery_worker",
            worker_id=self.worker_id,
        )

    async def run_forever(self) -> None:
        if not self.settings.notification_worker_enabled:
            self.logger.info("notification_worker_disabled", extra={"worker_id": self.worker_id})
            return
        self.logger.info("notification_worker_started", extra={"worker_id": self.worker_id})
        ...
        try:
            while not self.stopping.is_set():
                ...
                try:
                    ...
                    self.logger.exception(
                        "notification_worker_poll_failed",
            ...
            self.logger.info("notification_worker_stopped", extra={"worker_id": self.worker_id})

    async def poll_once(self) -> int:
        self.logger.info(
            "notification_worker_poll_started",
            extra={
                "worker_id": self.worker_id,
                "batch_size": self.settings.notification_worker_batch_size,
            },
        )
        async with self.session_factory() as session:
            service = NotificationService(session, settings=self.settings)
    ...
    def next_sleep_seconds(self, claimed_count: int) -> float:
        base_seconds = self.settings.notification_worker_poll_seconds
        if claimed_count:
            return base_seconds
        jitter = self.settings.notification_worker_jitter_seconds
        if jitter <= 0:
            return base_seconds
