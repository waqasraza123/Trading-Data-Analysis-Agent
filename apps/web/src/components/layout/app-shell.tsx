...
import { primaryWorkflowTargets, workflowHref, workflowTargets } from "./workflow-links";
...
export function AppShell({ appName, children }: AppShellProps) {
  const navigationItems = primaryWorkflowTargets.map((target) => ({
    href: workflowHref(target),
    label: workflowTargets[target].label,
  }));
