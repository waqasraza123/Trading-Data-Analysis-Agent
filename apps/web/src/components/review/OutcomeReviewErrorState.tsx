    ...
    <Panel title="Backend warnings" eyebrow="Scoped failures">
      <div className="space-y-2">
        {visibleFailures.map((failure) => (
          <div key={`${failure.label}-${failure.status}-${failure.message}`} className="rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:bg-amber-950 dark:text-amber-100">
