...
export function CommandCenterErrorState({
  ...
  return (
    <Panel title="Backend state" eyebrow="Availability">
      {backendUnavailable && (
      ...
      {visibleFailures.length > 0 ? (
        <div className="space-y-2">
          {visibleFailures.map((failure) => (
            <div key={`${failure.label}:${failure.status}:${failure.message}`} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-[var(--line)] p-3">
              <div>
                <p className="text-sm font-semibold text-[var(--strong)]">{failure.label}</p>
                <p className="text-sm text-slate-500">{failure.message}</p>
              </div>
              <Badge value={failure.status === 0 ? "Network" : String(failure.status)} tone={failure.status === 0 ? "danger" : "warning"} />
            </div>
          ))}
        </div>
      ) : (
