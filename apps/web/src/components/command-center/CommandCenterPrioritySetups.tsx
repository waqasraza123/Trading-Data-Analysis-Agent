...
export function CommandCenterPrioritySetups({ data }: { data: CommandCenterData }) {
  return (
    <Panel title="Review first" eyebrow="Priority setups" className={motionRevealPresetClass()}>
          {data.reviewFirst.length === 0 ? (
            ...
            <div className="grid gap-3 xl:grid-cols-2">
              {data.reviewFirst.map((item, index) => (
                <AnimatedListItem key={item.signalId} as="article" style={motionRevealDensityStyle(index, "compact")}>
                  <Link
                    href={item.href}
                    className={`rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 ${motionCardClass} ${MOTION_INTERACTIVE_CLASS}`}
                  >
