...
export function CommandCenterMorningBrief({ data }: { data: CommandCenterData }) {
  return (
    <Panel title="What changed?" eyebrow="Morning brief" className={motionRevealPresetClass()}>
      {data.whatChanged.length === 0 ? (
        <p className="text-sm text-slate-500">{data.sectionStatuses.whatChanged.message}</p>
      ) : (
        <div className="space-y-3">
          {data.whatChanged.map((item, index) => (
            <AnimatedListItem key={item.id} as="article" style={motionRevealDensityStyle(index, "compact")}>
              <Link
                href={item.href}
                className={`block rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 transition ${motionCardClass} ${MOTION_INTERACTIVE_CLASS}`}
              >
