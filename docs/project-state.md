...
- Production auth and workspace RBAC exist under `apps/api/app/modules/auth/` and `apps/api/app/modules/permissions/`. `AUTH_MODE=dev` with `AUTH_ENABLED=false` remains the local/test pass-through default; production modes support first-party password sessions backed by Postgres, user-owned password rotation plus session inventory/revocation APIs, legacy admin API-key compatibility, hashed persisted API keys, RS256 JWT identity lookup through `auth_identities`, workspace membership enforcement, permission dependencies, and `/auth/me` plus `/auth/context` without adding broker execution, auto-trading, copy trading, or financial-advice behavior.
