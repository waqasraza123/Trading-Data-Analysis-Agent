...
- Equity data background operations are implemented with `equity_data_operations`, the
  ...
  selected operation detail, row-level import error review, and safe stop controls for
  pending/running operations. Raw CSV file bytes and raw provider secrets are not persisted.
