...
- The first Go sidecar exists under `apps/go/market-worker` as an additive market data worker over
  ...
  handling, bounded parallel execution, claimed job lock renewal, and an operations runbook. Python
  remains the canonical API/product brain and no Python provider polling path is removed.
