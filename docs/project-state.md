...
- Chart screenshot ingestion accepts manually or externally extracted OHLC rows and deterministic PNG candlestick image uploads, supports write-free PNG extraction preview, stores valid extracted rows through the shared candle path, persists deterministic trend hypotheses, supports human review/correction without mutating the original extraction audit trail, can optionally trigger the existing deterministic analysis lifecycle over the extracted candle window, and exposes a decision endpoint that returns either analysis-backed reasoning or screenshot-hypothesis fallback reasoning.
