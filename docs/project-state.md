...
- Chart screenshot ingestion accepts manually or externally extracted OHLC rows and deterministic PNG candlestick image uploads, stores valid extracted rows through the shared candle path, persists deterministic trend hypotheses, and can optionally trigger the existing deterministic analysis lifecycle over the extracted candle window.
...
- Implemented chart_screenshot_runs migration/model, chart_screenshot data source type, screenshot-derived candle storage linkage, deterministic trend-hypothesis service, manual/external OHLC APIs, deterministic PNG candlestick extraction API, optional analysis-run triggering, and documentation.
