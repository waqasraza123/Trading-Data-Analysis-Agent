...
- The first Go sidecar exists under `apps/go/market-worker` as an additive market data worker over
  ...
  breaking, claimed job lock renewal, bounded job/request execution timeouts, and an operations
  runbook. Python remains the canonical API/product brain and no Python provider polling path is
  removed.
