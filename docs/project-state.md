...
- Production auth and workspace RBAC exist under `apps/api/app/modules/auth/` and `apps/api/app/modules/permissions/`. `AUTH_MODE=dev` with `AUTH_ENABLED=false` remains the local/test pass-through default; production modes support first-party password sessions backed by Postgres, user-owned password rotation plus session inventory/revocation/activity APIs, hashed auth activity events, legacy admin API-key compatibility, hashed persisted API keys, RS256 JWT identity lookup through `auth_identities`, workspace membership enforcement, permission dependencies, and `/auth/me` plus `/auth/context` without adding broker execution, auto-trading, copy trading, or financial-advice behavior.
...
- The first Go sidecar exists under `apps/go/market-worker` as an additive market data worker over
  ...
  eligible rows as `stale` for operator visibility before skipping them from claim cycles. The first
  heartbeat after a stale period transitions each subscription back to `active` and increments
  `liveSubscriptionsRevived` in `/metrics.json`, so operators can confirm recovery.
  ...
  checks, and an operations runbook. Production-grade runtime live subscription ingestion is now
  implemented in `apps/go/market-worker` when `jobs` mode and live stream settings are enabled:
  candidates are claimed with database leases, websocket streams are consumed with reconnection and
  heartbeat tracking, and stops/pauses release lease ownership promptly. Python remains the
...
- Daily Trading Dashboard web app exists under `apps/web` with `/login`, `/register`, `/account`, `/command-center`, `/dashboard`, `/brief`, `/triage`, `/scanner`, `/data/onboarding`, `/quality`, `/signals/[signalId]`, `/symbols/[symbolId]`, `/preferences/strategy`, `/review/outcomes`, and `/journal` routes. It uses `NEXT_PUBLIC_API_BASE_URL` and `NEXT_PUBLIC_APP_NAME`, a typed fetch wrapper with timeout/error handling, safe 404/missing-endpoint fallbacks, reusable auth/account/dashboard/detail/brief/onboarding/scanner/preference/review/journal/quality components, and non-advisory market-intelligence copy. The `/account` route manages first-party password rotation, session inventory, recent auth activity, and safe revocation through the same-origin proxy without exposing bearer tokens to browser JavaScript. The `/signals/[signalId]` route is now a full read-only setup detail view that prefers intelligence reports and falls back to setup context, evidence, outcomes, readiness, quality gates, scenario reasoning, historical cases, multi-timeframe context, cross-asset context, audit timeline, and journal endpoints.
- First-run onboarding exists under backend `/onboarding` and web `/onboarding` as a read-only status composition plus explicit safe setup actions, workspace selector support, command-center readiness banner, and demo workspace option. It reuses product readiness, workspace setup, demo mode, seed, symbol, source, watchlist, scanner, and daily workflow modules without hidden provider polling, broker execution, auto-trading, external notifications, or financial-advice behavior.
- The web app now has a Tailwind-native cockpit design system under `apps/web/src/components/ui`, `apps/web/src/components/layout`, `apps/web/src/components/status`, and `apps/web/src/lib/ui`, with responsive sidebar/topbar/mobile navigation, workspace context, safe API health status display, reusable surfaces, badges, state components, and safe-label helpers. The design system remains read-only and does not introduce broker execution, auto-trading, alerts, or financial-advice output.
...
- Motion rollout governance in `apps/web/docs/motion-ui.md` now includes typed density profiles, route manifests, and a production checklist so future UI surfaces inherit the same reveal rhythm without changing backend behavior.
- Motion API migration is now complete for shared web components: legacy `motionRevealClass()` usage in command center, brief, triage, and shell primitives has moved to preset-based `motionRevealPresetClass()` while preserving `motionRevealClass` compatibility.
- Motion compatibility helpers remain available for migration only and are now explicitly deprecated in code docs to keep future production additions on the preset/density/profile API.
- Motion compatibility helper invocations now emit one-time dev warnings, enabling easier migration audits in development and review without any runtime behavior changes in production traffic.
- Motion rollout governance now includes a route audit script (`apps/web/scripts/motion-rollout-audit.mjs`) and `apps/web/package.json` script `motion:rollout-audit` so route-level rollout coverage can be checked before merge.
- Motion rollout governance now enforces the audit via a manifest (`apps/web/scripts/motion-rollout-manifest.json`) and supports strict JSON/legacy-guard output modes from `motion:rollout-audit` and `motion:rollout-audit:json`.
- Motion rollout governance now supports markdown report generation in strict mode via `motion:rollout-audit:report` and `--report`/`--report-path`, with output tracked at `apps/web/docs/motion-rollout-audit-report.md` by default.
- Non-JSON invocation also supports `--report` for local markdown artifact-only exports while preserving strict JSON output in production-facing commands.
- Motion rollout governance now also performs drift coverage validation by scanning `apps/web/app/*/page.tsx` against the manifest and failing when discovered routes are untracked (with explicit, temporary `--allow-coverage-gaps` override).
- Manifest schema is now versioned and validates required route/page/requires fields plus optional `revealDensity`; strict mode (`motion:rollout-audit:strict` / `--strict`) enforces strict validation and route coverage/legacy checks.
- Command-center and triage interaction polish now uses a shared `MOTION_INTERACTIVE_CLASS` token for consistent `motion-hover-lift` + `focus-visible` behavior on interactive rows/links, with preserved data contracts and read-only safety postures.
- App-shell navigation surfaces now inherit the same interaction token in `Sidebar` and `MobileNav`, so top-level command and route pills keep consistent motion-hover-fallback and explicit `focus-visible` states across desktop and mobile layouts.
- Workspace selector inputs now participate in the motion interaction contract by using `MOTION_INTERACTIVE_CLASS` in `WorkspaceSelector`, keeping keyboard-focus and hover behavior consistent in desktop and mobile shells.
- Shared interaction polish is now consolidated in additional core primitives (`Card`, `Surface`, `Badge`, and `Button`) by using `MOTION_INTERACTIVE_CLASS` in interactive modes so focus-visible and hover-lift behavior remains centralized and production-consistent.
- Remaining motion-aware high-level wrappers are now consolidated (`MetricCard`, `Section`, and `PageHeader`) on `MOTION_INTERACTIVE_CLASS` so the hover/focus polish set remains consistent across static and interactive primitive surfaces.
- Final interaction normalization completed by removing direct `motion-hover-lift` from non-interactive paths in `Card`, `Surface`, and `Badge`, leaving motion-hover/focus effects only through explicit interactive composition with `MOTION_INTERACTIVE_CLASS`.
- Loading placeholder primitives now route legacy `Skeleton` usage through the shared `ShimmerSkeleton` implementation, keeping placeholder animation styles consistent across routes while preserving accessibility defaults.
- Route-level loading behavior was standardized by adding `apps/web/src/components/layout/RouteLoadingShell.tsx` and migrating all product-route `loading.tsx` files to this shared AppShell wrapper for consistent motion-safe suspense skeleton scaffolding.
- Scanner now has a route-level loading boundary (`apps/web/app/scanner/loading.tsx`) using the shared loading shell to keep first-load transitions visually consistent and low-risk.
- Scanner route reveal polish now applies staged `AnimatedListItem` transitions in `apps/web/app/scanner/page.tsx` for hero, controls, workflow/result panels, and list/error blocks while preserving all data contracts.
- Visual setup chart panels exist on `/signals/[signalId]` as SVG-only read-only market-intelligence context. They compose existing analysis-run, final candle, candle quality, setup context, outcome, advanced-feature, and report endpoints; show recent final candles, signal window, observation zones, invalidation context, target context zones, support/resistance context, latest final candle, review marker, setup quality/freshness/data-quality badges, warnings, and observed outcome markers; and do not add charting dependencies, backend mutations, broker workflows, auto-trading, or financial-advice output.
- Component-level production reveal polish now covers `symbols/[symbolId]` and `data/onboarding` internals:
  - `SymbolDetailView` now staggers section/panel and row-level reveals for memory cards, latest signal rows, scan cards, and analysis-run rows.
  - `OnboardingWorkflow` now staggers hero/header, provider health panel, step navigation, active step panel, and bottom controls.
- This rollout keeps all data composition and read-only safety contracts unchanged while expanding the same shared motion API coverage via `@/lib/ui/motion`.
- `/triage` and `/signals/[signalId]` now form a premium setup review workflow in the web layer. Triage uses the six visible buckets Review First, Needs Confirmation, Conflicted, Avoid / No Directional, Stale / Data Issue, and Review Required with sticky filters, symbol search, preference-profile scoping, priority sorting, compact evidence/risk/outcome cards, and safe optional-endpoint fallbacks. Signal detail uses feature-local setup review components for a sticky setup header, visual context, evidence/confidence, wait/avoid, historical context, reasoning, journal, audit, quality-gate, multi-timeframe, and cross-asset sections. The workflow remains review-only and does not add broker execution, trade instructions, account-result metrics, or financial-advice output.
...
- High-volume candle ingestion performance tracking exists under `apps/api/app/modules/candle_ingestion_performance/` and `/candle-ingestion`. CSV imports, JSON imports, and provider polling use chunked validation and batch candle storage diagnostics while preserving existing final/partial candle source-of-truth rules and default final-candle analysis semantics.
...
- Daily routine templates and runner exist under `apps/api/app/modules/daily_routines/` and `/daily-routines`. They seed named operator routines for pre-market, session-open, crypto 24h, close-of-day, stale-data repair, outcome review, quality review, and journal follow-up flows. Routine runs persist explicit bounded step records over backend-safe deterministic services only and do not place orders, call brokers, auto-trade, suggest directional order actions, deliver external notifications by default, or provide financial advice.
- Demo mode exists under `apps/api/app/modules/demo_mode/`, `/demo-mode`, and `apps/web/app/demo/page.tsx` for local/staging product smoke validation. It can seed a labeled demo workspace, demo symbols, synthetic fixture candles, JSON imports, deterministic analysis runs, signals, setup context, priority scores, observed outcomes, scanner artifacts, a daily brief, optional journal note, and readiness output without external providers, broker execution, auto-trading, production data, or financial-advice output.
- Daily Trading Command Center exists at `apps/web/app/command-center/page.tsx` as the default start page. It is a premium Tailwind-native daily cockpit over existing provider health, provider polling, product readiness, signal priority, preference profile, brief, triage, scanner, journal, market memory, signal digest, outcome, readiness, review, action item, workflow, runtime, and health clients. It turns the existing pages into a daily workflow for data readiness, review-first setups, confirmation needs, avoid/no-directional contexts, outcome review, workflow progress, notifications/review items, and navigation. It does not add backend routes, broker execution, auto-trading, external notifications, or financial-advice output.
- The five modern web UI/productization branches are integrated into one cohesive Tailwind-native product shell. `apps/web` now has one canonical app shell/navigation registry, shared UI/status primitives, typed fail-soft API clients, and coordinated page experiences for command center, brief, data onboarding, scanner, triage, setup detail, journal, outcome review, quality, notifications, dashboard, and preferences. Navigation preserves workspace context where available and the UI remains read-only and non-advisory.
- Product readiness checklist exists under `apps/api/app/modules/product_readiness/` and `apps/web/app/readiness/page.tsx`. It persists explicit readiness runs for API, DB, migrations, seed/workspace/user setup, symbols, data sources, provider credentials/health, candle freshness, watchlists, scan configs, daily workflow availability, worker status, optional notifications, journal readiness, web API configuration, and critical stale/missing data indicators. It validates readiness only and does not seed data, run scans, run daily workflows, start workers, call providers, send alerts, execute broker actions, auto-trade, or provide financial advice.
- Guided workspace setup exists under `apps/api/app/modules/workspace_setup/`, `/workspace-setup`, and `apps/web/app/setup/page.tsx`. It persists setup runs and step results for workspace, operator, symbols, data source, credential reference metadata, watchlist, scanner preset, preference profile, optional synthetic demo candles, readiness validation, and an optional explicit first deterministic scan. It does not store raw secrets, connect to brokers for orders, place trades, auto-trade, send trading alerts, run hidden scans, or provide financial advice.
...
- The `/journal`, `/review/outcomes`, `/quality`, and `/notifications` web routes use polished daily review surfaces with a shared Tailwind review UI kit, consistent shell metrics, filters, tables, source links, strong empty states, and safe non-advisory labels. The journal supports optional symbol/timeframe filtering when analysis-run context is available; the outcome review route uses `apps/web/src/components/outcome-review/` for queue and diagnostic composition; the quality route includes a what-to-review focus panel; the notification route remains an in-app event inbox with read/acknowledge/archive actions only when backend APIs support them.
...
- Workspace daily brief composition exists in the web layer under `apps/web/src/lib/api/brief.ts` and `apps/web/src/lib/brief/`. It now prefers `GET /workspaces/{workspace_id}/daily-brief/latest` and falls back to existing optional backend endpoints when the persisted backend brief is missing or unavailable. `/brief` renders a structured narrative summary with period, watchlist/filter context, summary stats, what changed, fresh symbols, review-first setups, confirmation needs, avoid conditions, outcomes ready, watch-next context, backend-safe actions, data-quality context, and graceful empty/backend-unavailable states.
...
- Implemented visual setup chart panels in `apps/web` with reusable SVG candlestick components, chart scale/overlay utilities, setup detail API composition, signal detail integration, and documentation.
- Implemented demo mode product smoke flow with guarded backend APIs, CLI entrypoint, deterministic synthetic candle import, analysis/signal/context/priority/outcome/scan/brief/journal/readiness artifact creation, web `/demo` page, and docs.
- Implemented workspace overview and workspace quick-action APIs plus command-center overview integration. The daily cockpit now prefers one backend-composed read-only workspace payload, exposes explicit backend-safe deterministic actions, and keeps frontend fallback composition for local/dev or partially available modules.
...
- Redesigned `/scanner` and `/data/onboarding` as guided Tailwind-native workflows with scanner health metrics, preset and run-now flows, watchlist/config management, provider health matrix, credential status, gap recovery planning, and safe non-advisory copy.
...
- The workspace RBAC layer uses a static role-to-permission registry for now instead of a database table. Roles are still stored on existing `users.role`; dynamic permission persistence should wait until there is a real identity provider and role-management surface. Permission dependencies must not force OAuth/JWT, must preserve `AUTH_ENABLED=false` local/test behavior, and must not expose secrets in audit logs.
...
- Reasoning action plans may persist and execute only backend-safe follow-up work. Order instructions, position management, leverage requests, copy-trading requests, or trade execution requests are rejected and never persisted as executable items.
...
- Equity data background operations are implemented with `equity_data_operations`, the
  ...
  audit bundle, recovery plan, retry readiness, retry lineage, operation diagnostics timeline,
  row-level import error review, and safe stop/retry controls for pending, running, warning, failed,
  or cancelled operations. Retry readiness preflights replayable payload availability, provider
  readiness, and queued/sync mode feasibility without creating work. Recovery plans compose safe
  next-step guidance from diagnostics, retry readiness, row errors, and review context without
  executing actions. The audit bundle composes detail, diagnostics, lineage, retry readiness,
  recovery plan, row errors, and optional review context into a read-only operator package. Retry
  creates a new operation only from replayable persisted payloads and rejects compacted row
  summaries. Raw CSV file bytes and raw provider secrets are not persisted.
