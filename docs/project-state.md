...
- Daily Trading Dashboard web app exists under `apps/web` with `/dashboard`, `/data/onboarding`, `/signals/[signalId]`, and `/symbols/[symbolId]` routes. It uses `NEXT_PUBLIC_API_BASE_URL` and `NEXT_PUBLIC_APP_NAME`, a typed fetch wrapper with timeout/error handling, safe 404/missing-endpoint fallbacks, reusable dashboard/detail/onboarding components, and non-advisory market-intelligence copy.
