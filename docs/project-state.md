...
- Daily Trading Dashboard web app exists under `apps/web` with `/dashboard`, `/brief`, `/triage`, `/scanner`, `/data/onboarding`, `/signals/[signalId]`, and `/symbols/[symbolId]` routes. It uses `NEXT_PUBLIC_API_BASE_URL` and `NEXT_PUBLIC_APP_NAME`, a typed fetch wrapper with timeout/error handling, safe 404/missing-endpoint fallbacks, reusable dashboard/detail/brief/onboarding/scanner components, and non-advisory market-intelligence copy. The `/signals/[signalId]` route is now a full read-only setup detail view that prefers intelligence reports and falls back to setup context, evidence, outcomes, readiness, quality gates, scenario reasoning, historical cases, multi-timeframe context, cross-asset context, audit timeline, and journal endpoints.
- Signal Triage Board web route exists under `apps/web/app/triage/page.tsx` as a read-only frontend composition over existing signal, market memory, setup context, outcome, readiness, report, quality, reasoning, review, and action item APIs. It does not add a backend triage endpoint, mutate artifacts, rank trades, execute broker actions, or provide financial advice.
- Workspace daily brief composition exists in the web layer under `apps/web/src/lib/api/brief.ts` and `apps/web/src/lib/brief/`. It composes existing optional backend endpoints into a read-only operator brief for top symbols, freshness, bias, setup context, avoid conditions, outcomes, backend action items, watch-next observations, data-quality issues, review-needed items, and recent digest summaries without adding a backend brief endpoint.
- The daily web workflow is now integrated through shared navigation and workspace-aware links: verify data freshness in `/data/onboarding`, run deterministic watchlist scans in `/scanner`, review the workspace brief in `/brief`, triage signals in `/triage`, inspect setup context in `/signals/[signalId]`, add observational journal notes when useful, and evaluate observed outcomes through brief/dashboard/detail views. These pages do not trigger notifications, broker execution, auto-trading, copy trading, or financial-advice output.
