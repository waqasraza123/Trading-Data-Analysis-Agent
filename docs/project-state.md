...
- Trading journal feedback is implemented downstream of deterministic signals and outcomes. It persists user decision notes, optional artifact attachments, and deterministic reflection rows using safe labels for outcome comparison without mutating signals or outcomes, calculating PnL, importing broker data, or providing financial advice.
...
- Setup context is structured market-intelligence context only. Invalidation context is not a stop-loss instruction, observation zones are not entries, target context is not take-profit guidance, and next observations must remain backend-safe review steps.
- Trading journal feedback is a user/operator learning layer only. It may compare a user decision note with stored deterministic outcomes, but it must not mutate signal classification, mutate outcomes, calculate PnL, import broker records, create broker workflows, trigger alerts, or provide financial advice.
